...
Echo.private('lobby')
    .listen('GameJoined', (event) => {
        ...
        if (games.value.length < 5) {
            router.reload({ only: ['games'], onSuccess: () => games.value = $page.props.games.data });
        }
    });
</script>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg py-4 px-6">
                    ...
                    <ul class="divide-y mt-6">
                        <li v-for="game in games" :key="game.id" class="px-2 py-1.5 flex justify-between items-center">
                            ...
                            <Link :href="route('games.join', game)"  method="post" as="button" class="hover:bg-gray-50 p-2 rounded transition-colors">Join Game</Link>
                        </li>
                    </ul>
                </div>
            </div>
